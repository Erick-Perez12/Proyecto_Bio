...
def get_clustering_levels(dist_matrix, Z):
    ...
    for i, (cluster1, cluster2, dist, _) in enumerate(Z):
        ...
        result += (
            f'Nivel {i+1}: Cluster {"".join(cluster_labels[int(cluster1)])} se une con '
            f'Cluster {"".join(cluster_labels[int(cluster2)])} a una distancia de {dist:.4f}\n'
        )

        # Actualiza el mapeo de clusters
