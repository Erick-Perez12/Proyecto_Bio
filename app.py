...
from algorithms.mayusculas import transform_to_uppercase  
from algorithms.mayusculas import transform_to_lowecase  
from algorithms.coincidencias import find_coincidences  
from algorithms.identificador_secuencia import identificar_secuencia  
from algorithms.transcripcion import transcripcion_adn_arn  
from algorithms.sub_cadena import buscar_subcadena  
from algorithms.sub_cadena import calcular_score 
from algorithms.alineamiento_global import needleman_wunsch_score, globalTraceback  
...
from algorithms.matriz_punto import dot_matrix, plot_dot_matrix  
...
def analizar():
    ...
    if operation == 'identificar_secuencia':
        ...
        aligned_sequences = align_to_center(sequences, index, match, mismatch, gap)
        center_seq = sequences[index].ljust(max(len(seq) for seq in sequences), '-')
        msa = merge_alignments(center_seq, aligned_sequences)
        print(type(result)) 
        return render_template('star_alignment.html',
                            sequences=sequences,
                            find_secuencia_central=find_secuencia_central,
